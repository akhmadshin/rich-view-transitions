const VIEW_TRANSITION_NAME = '__VTH_transition-name';
let key = 'initial';
...
export const handleHistoryTransitionStarted = (nextRouterKey: string = 'initial') => {
  ...
  const transitionElementSelector = sessionStorage.getItem(`__VTH_view_transition_element_selector_${routerKey}-${nextRouterKey}`) || '';
...
export const handleRouteChangeComplete = (_key?: string) => {
  ...
  if (transitionEndElementSelector) {
    sessionStorage.setItem(`__VTH_view_transition_element_selector_${previousKey}-${key}`, transitionEndElementSelector);
    transitionEndElementSelector = undefined;
  }
  const backRouterKey = `${key}-${previousKey}`;
  const transitionElementSelector = sessionStorage.getItem(`__VTH_view_transition_element_selector_${backRouterKey}`);
